...
export default function CardManagement({ onAddCard }: CardManagementProps) {
  ...
  const filteredCards = cardList.filter((card: CardWithReview) => {
    ...
    const matchesSubject =
      selectedSubject === "全部" || card.subject === selectedSubject;
                ...
                filteredCards.map((card: CardWithReview) => (
                  <tr key={card.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {card.lastReview ? (
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getFamiliarityClass(card.lastReview.familiarityScore)}`}
                        >
                          <span className="ri-fire-fill mr-1">🔥</span>{" "}
                          {card.lastReview.familiarityScore}
                        </span>
                      ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {card.lastReview
                        ? format(
                            new Date(card.lastReview.reviewDate),
                            "yyyy-MM-dd",
                          )
                        : "–"}
                    </td>
                      ...
                      {card.lastReview ? (
                        <>
                          <div className="text-sm text-primary-600">
                            {format(
                              new Date(card.lastReview.nextReviewDate),
                              "yyyy-MM-dd",
                            )}
                          </div>
